...
namespace McpUnity.Tools
{
    ...
    public class AddAssetToSceneTool : McpToolBase
    {
        ...
        public override JObject Execute(JObject parameters)
        {
            ...
            long? parentId = parameters["parentId"]?.ToObject<long?>();
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(parentPath) || parentId.HasValue)
                {
                    ...
                    {
                        parent = EditorUtility.EntityIdToObject(EntityId.FromULong((ulong)parentId.Value)) as GameObject;
                    }
            ...
            return new JObject
            {
                ["success"] = true,
                ["type"] = "text",
                ["message"] = $"Successfully added asset '{asset.name}' with instance ID {(long)EntityId.ToULong(instance.GetEntityId())} to the scene",
                ["instanceId"] = (long)EntityId.ToULong(instance.GetEntityId())
            };
        }
    }
}
