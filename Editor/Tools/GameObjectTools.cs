...
namespace McpUnity.Tools
{
    ...
    public static class GameObjectToolUtils
    {
        ...
        public static JObject FindGameObject(long? instanceId, string objectPath, out GameObject gameObject, out string identifierInfo)
        {
            gameObject = null;
            identifierInfo = "";
    ...
    public class DuplicateGameObjectTool : McpToolBase
    {
        ...
        public override JObject Execute(JObject parameters)
        {
            ...
            {
                newParent = EditorUtility.EntityIdToObject(EntityId.FromULong((ulong)newParentId.Value)) as GameObject;
                if (newParent == null)
                {
                    return McpUnitySocketHandler.CreateErrorResponse(
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    ["instanceId"] = (long)EntityId.ToULong(duplicate.GetEntityId()),
                    ["name"] = duplicate.name,
    ...
    public class ReparentGameObjectTool : McpToolBase
    {
        ...
        public override JObject Execute(JObject parameters)
        {
            ...
            {
                GameObject newParent = EditorUtility.EntityIdToObject(EntityId.FromULong((ulong)newParentId.Value)) as GameObject;
                if (newParent == null)
                {
                    return McpUnitySocketHandler.CreateErrorResponse(
                        $"New parent GameObject not found with instance ID {newParentId.Value}.",
            ...
            if (moveToRoot && oldParent == null)
            {
                return new JObject
                {
                    ...
                    ["instanceId"] = (long)EntityId.ToULong(targetObject.GetEntityId()),
